######################################################
# Just same as the resnet detailed in spikingjelly
# but here i only need the feature extract part so...

# -> In addition
# -> the SNN's resnet (ResNet-19) for cifar tend to use only two times' expandation
# -> But the resnet used in DER for cifar100, it use the standard resnet while only shift the first convnet
...
class SpikingResNet(nn.Module):
    def __init__(
        self, block, layers, zero_init_residual=False, groups=1, width_per_group=64,
...
def _spiking_resnet(arch, block, layers, pretrained, spiking_neuron, **kwargs):
...
def spiking_resnet18(pretrained=False, spiking_neuron: callable=None, **kwargs):
    return _spiking_resnet('spiking_resnet18', BasicBlock, [2, 2, 2, 2], pretrained, spiking_neuron=spiking_neuron, **kwargs)
