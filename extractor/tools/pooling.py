...
# -> this pooling process is to get the <kmax> largest values and <kmin> lowest values in every column
# -> and output their average (can be seen as intermediate form of Average pooling and max pooling).
class WeldonPool2d(nn.Module):
    def __init__(self, kmax=1, kmin=None, **kwargs):
        ...
        print(f"Using Weldon Pooling with kmax={self.kmax}, kmin={self.kmin}.")
    ...
    def _define_function(self):
        class WeldonPool2dFunction(Function):
            ...
            def forward(ctx, input):
                ...
                # -> .narrow() is used to split from index 0 with length kmax on the dim 2
                indices_max = indices.narrow(2, 0, kmax)
                output = sorted.narrow(2, 0, kmax).sum(2).div_(kmax)

                if kmin > 0:
                    # compute scores for min instance
                    indices_min = indices.narrow(2, n-kmin, kmin)
                    output.add_(sorted.narrow(2, n-kmin, kmin).sum(2).div(kmin)).div_(2)

                # save input for backward
                ctx.saved_for_backward(indices_max, indices_min, input)
            ...
            def backward(ctx, grad_output):
                ...
                # get the nunmber of max and min instances
                kmax = WeldonPool2dFunction.get_number_of_instances(self.kmax, n)
                kmin = WeldonPool2dFunction.get_number_of_instances(self.kmin, n)

                # calculate gradient for max instances
                ...
                # -> scatter() is used to cover the vector based on input index
                ...
                if kmin > 0:
                    ...
                    grad_input_min = grad_output.new().resize_(batch_size, num_channels, n).fill_(0).scatter(2, indices_min, grad_output_min).div_(kmin)
