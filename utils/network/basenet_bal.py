...
class BasicNet_bal(nn.Module):
    def __init__(
        ...
        super(BasicNet_bal, self).__init__()
        self.init = init
    ...
    def _add_classes_multi_fc(self, n_classes):
        if self.ntask > 1:
            new_conv = factory.get_convnet(self.convnet_type,
                                          remove_last_relu=self.remove_last_relu).to(self.device)
            # -> copy the parameters of the old extractor #############
            new_conv.load_state_dict(self.convnets[-1].state_dict())
            ###########################################################
            self.convnets.append(new_conv)
    ...
    def finetune_parameters(self):
        ...
        return [p for param_group in params_to_update for p in param_group]
