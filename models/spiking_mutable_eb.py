...
class Spiking_Mutable(IncrementalLearner):
    effect_fac = 8
    change_fac = 4
    ...
    def __init__(self, cfg, inc_dataset=None, tensorboard=None):
        ...
        # Augment config
        self._aug_config = cfg["aug_config"]
        logger.info(f"the augment setting is {self._aug_config}")
        # other setting
        ...
        self.recover_old = cfg['recover_old']  # -> whether to recover the
        ...
        logger.info(f"supp_o2n: {self.supp_o2n}, recover_old: {self.recover_old}, use_full_kl: {self.kl_config}")
    ...
    def _train_first_task(self, train_loader, val_loader):
        ...
        for epoch in range(self._n_epochs):
            ...
            if self._warmup:
                ...
                if epoch == self._cfg['warmup_epochs']:
                    if not self._cfg["orthogonal_init_clf"]:
        ...
        utils.display_weight_norm(logger, self._parallel_network, self._increments, "After raw training")
    ...
    def _train_latter_task(self, train_loader, val_loader):
        ...
        ######################################
        # Set the augment parameters
        meter_dict = {
            'main_align': None, 'main_old_summ': None, 'main_new_summ': None,
            'aux_align': None, 'aux_old_summ': None, 'aux_new_summ': None,
        }
        if self._aug_config["main_enable"]:
            meter_dict['main_align'] = 0.
            main_last_aug = False
            main_st_diff = 0.
            main_last_diff = 0.
            main_max_change = 0.
        if self._aug_config["aux_enable"]:
            meter_dict['aux_align'] = 0.
            aux_last_aug = False
            aux_st_diff = 0.
            aux_last_diff = 0.
            main_aux_change = 0.
        ...
        for epoch in range(self._n_epochs):
            ...
            ######################################
            # The assignment should be done at every beginning of the epoch should be put here
            if self._aug_config["main_enable"] and epoch >= self._aug_config['aug_st_ep']:
                meter_dict['main_old_summ'] = 0.
                meter_dict['main_new_summ'] = 0.
            if self._aug_config["aux_enable"] and epoch >= self._aug_config['aug_st_ep']:
                meter_dict['aux_old_summ'] = 0.
                meter_dict['aux_new_summ'] = 0.
            ######################################
            ...
            for i, (inputs, targets) in enumerate(train_loader):
                ...
                loss_ce, loss_aux, loss_ex = self._compute_combined_loss(
                    ...
                    epoch=epoch, meter_dict=meter_dict, accu=accu,
                )
            ...
            ######################################
            # Calculate the average logit info and set the align
            if epoch >= self._aug_config['aug_st_ep'] and meter_dict['main_old_summ'] is not None:
                avg_main_old = meter_dict['main_old_summ'] / len(self._inc_dataset.data_memory)
                avg_main_new = meter_dict['main_new_summ'] / len(self._inc_dataset.data_cur)
                # diff = (avg_main_new - avg_main_old) / slow_factor if avg_main_new > avg_main_old else avg_main_new - avg_main_old
                # meter_dict['main_align'] = max(meter_dict['main_align'] + diff, 0)
                diff = abs(avg_main_new - avg_main_old)
                if avg_main_new - avg_main_old > 0:
                    if not main_last_aug or (main_st_diff - diff > main_st_diff / self.effect_fac and main_last_diff - diff < main_max_change / self.change_fac):
                        meter_dict['main_align'] += diff
                        main_last_aug = True
                        main_max_change = 0.
                        main_st_diff = diff
                    else:
                        main_max_change = max(main_max_change, main_last_diff - diff)
                else:
                    if main_last_aug or (main_st_diff - diff > main_st_diff / self.effect_fac and main_last_diff - diff < main_max_change / self.change_fac):
                        meter_dict['main_align'] -= diff
                        main_last_aug = False
                        main_max_change = 0.
                        main_st_diff = diff
                ...
                main_last_diff = diff
                if (epoch + 1) % 3 == 0:
                    logger.info(f"avg_main_old: {round(avg_main_old, 2)}, avg_main_new: {round(avg_main_new, 2)}, logit_align: {round(meter_dict['main_align'], 2)}")
            if epoch >= self._aug_config['aug_st_ep'] and meter_dict['aux_old_summ'] is not None:
                avg_aux_old = meter_dict['aux_old_summ'] / len(self._inc_dataset.data_memory)
                avg_aux_new = meter_dict['aux_new_summ'] / len(self._inc_dataset.data_cur)
                # diff = (avg_aux_new - avg_aux_old) / slow_factor if avg_aux_new > avg_aux_old else avg_aux_new - avg_aux_old
                # meter_dict['aux_align'] = max(meter_dict['aux_align'] + diff, 0.)
                diff = abs(avg_aux_new - avg_aux_old)
                if avg_aux_new - avg_aux_old > 0:
                    if not aux_last_aug or (aux_st_diff - diff > aux_st_diff / self.effect_fac and aux_last_diff - diff < aux_max_change / self.change_fac):
                        meter_dict['aux_align'] += diff
                        aux_last_aug = True
                        aux_max_change = 0.
                        aux_st_diff = diff
                    else:
                        aux_max_change = max(aux_max_change, aux_last_diff - diff)
                else:
                    if aux_last_aug or (aux_st_diff - diff > aux_st_diff / self.effect_fac and aux_last_diff - diff < aux_max_change / self.change_fac):
                        meter_dict['aux_align'] -= diff
                        aux_last_aug = False
                        aux_max_change = 0.
                        aux_st_diff = diff
                ...
                aux_last_diff = diff
                if (epoch + 1) % 3 == 0:
                    logger.info(f"avg_aux_old: {round(avg_aux_old, 2)}, avg_aux_new: {round(avg_aux_new, 2)}, logit_align: {round(meter_dict['aux_align'], 2)}")
            #####################################
    ...
    def _compute_combined_loss(
        ...
        epoch, meter_dict, accu=None
        # -> maybe will have others... though
    ):
        assert self._task > 0
        assert meter_dict is not None
        inputs, targets = inputs.to(self._device, non_blocking=True), targets.to(self._device, non_blocking=True)
        ...
        if epoch >= self._aug_config['aug_st_ep']:
            if self._aug_config['main_enable']:
                if torch.sum(old_classes) > 0:
                    old_outputs = freq_opts[old_classes]
                    old_targets = targets[old_classes]
                    old_corr = old_outputs[torch.arange(old_outputs.shape[0]), old_targets]
                    meter_dict['main_old_summ'] += torch.sum(old_corr).item()
                if torch.sum(new_classes) > 0:
                    new_outputs = freq_opts[new_classes]
                    new_targets = targets[new_classes]
                    new_corr = new_outputs[torch.arange(new_outputs.shape[0]), new_targets]
                    meter_dict['main_new_summ'] += torch.sum(new_corr).item()
        ...
        ce_loss = self._auged_ce_loss(
            inputs, outputs, 'logit', targets, epoch, old_classes, new_classes,
            kl_config=self.kl_config, aug_st_ep=self._aug_config['aug_st_ep'],
            aug_enable=self._aug_config['main_enable'],
            aug_range=self._aug_config['main_range'], aug_part=self._aug_config['main_part'],
            use_minus=self._aug_config['main_minus'], logit_align=meter_dict['main_align'],
        )
        ...
        if self.use_aux:
            ...
            aux_kl = self._aug_config['aux_kl']
            if self._cfg['aux_n+1']:
                aux_target[old_classes] = 0
                aux_target[new_classes] -= sum(self._inc_dataset.increments[:self._task]) - 1
                aux_kl = "none"

            mean_aux_opts = torch.mean(outputs['aux_logit'], dim=1)  # [B, T, N] -> [B, N]
            # -> collect the aux_info
            if epoch >= self._aug_config['aug_st_ep'] and self._aug_config['aux_enable']:
                if torch.sum(old_classes) > 0:
                    old_outputs = mean_aux_opts[old_classes]
                    old_targets = aux_target[old_classes]
                    old_corr = old_outputs[torch.arange(old_outputs.shape[0]), old_targets]
                    meter_dict['aux_old_summ'] += torch.sum(old_corr).item()
                if torch.sum(new_classes) > 0:
                    new_outputs = mean_aux_opts[new_classes]
                    new_targets = aux_target[new_classes]
                    new_corr = new_outputs[torch.arange(new_outputs.shape[0]), new_targets]
                    meter_dict['aux_new_summ'] += torch.sum(new_corr).item()

            aux_loss = self._auged_ce_loss(
                inputs, outputs, 'aux_logit', aux_target, epoch, old_classes, new_classes,
                kl_config=aux_kl, aug_st_ep=self._aug_config['aug_st_ep'],
                aug_enable=self._aug_config['aux_enable'],
                aug_range=self._aug_config['aux_range'], aug_part=self._aug_config['aux_part'],
                use_minus=self._aug_config['aux_minus'], logit_align=meter_dict['aux_align'],
            )
        else:
    ...
    def _after_task(self, taski, inc_dataset):
        ...
        if self._cfg['decouple']['enable'] and taski > 0:
            if self.recover_old:
                assert self._cfg['decouple']['block_fix'] == 'old'
                logger.info("Recover the old weight in finetune-step")
                self._network.reset_old_clf_weight(self._old_model.module.clf_weight)

            if not self._cfg["fix_clf"]:
    ...
    def _compute_accuracy_by_netout(self, data_loader):
        # Meanless part #####################
        # from ..extractor.spiking_resnet import collecting, neuron_cnt, spike_cnt
        # from extractor.spiking_resnet import neuron_cnt, spike_cnt
        collecting = True
        #####################################
        ...
        # Meanless part #####################
        collecting = False
        # logger.info(f"The whole neuron_cnt is {neuron_cnt}, The whole spike_cnt is {spike_cnt}")
        # logger.info(f"The whole spiking-rate is {spike_cnt/neuron_cnt}")
        #####################################
    ...
    def _auged_ce_loss(
        ...
        old_classes, new_clasees, kl_config, aug_st_ep,
        aug_enable, aug_range, aug_part, use_minus, logit_align
    ):
        ...
        auged_opts = logit_opts
        if aug_enable and epoch >= aug_st_ep and logit_align != 0.:
            opt_cls = logit_opts.shape[-1]
            if aug_range == 'old_sample':
                sample_mask = old_classes
            elif aug_range == 'new_sample':
                sample_mask = new_clasees
            elif aug_range == 'all_sample':
                sample_mask = torch.ones_like(old_classes).to(self._device, dtype=torch.bool)
            else:
                raise ValueError(f"<{aug_range}> is not a valid range type")

            if aug_part == 'former_logit':
                part_l, part_r = 0, opt_cls - self._task_size
            elif aug_part == 'latter_logit':
                part_l, part_r = opt_cls - self._task_size, opt_cls
            elif aug_part == 'all':
                part_l, part_r = 0, opt_cls
            else:
                raise ValueError(f"<{aug_part}> is not a valid logit-part type")

            # -> Here i decide to use minus/plus only rather than div/multi because of the similar effect
            aug_mtx = torch.zeros_like(logit_opts).to(self._device)
            if use_minus:  # [B, T, N]
                aug_mtx[sample_mask, :, part_l:part_r] = -logit_align
            else:
                aug_mtx[sample_mask, :, part_l:part_r] = logit_align
            # logit_opts = logit_opts + aug_mtx
            auged_opts = logit_opts + aug_mtx

        if self.TET_config['enable']:
            ce_loss = utils.TET_loss(auged_opts, targets, self.TET_config['TET_mean'], self.TET_config['TET_lamb'], reduction='none')
        else:
    ...
    def analyse_plotting(self):
        if self._task == 1 or self._task == 2:
            ...
            utils.get_step_logits(
                ...
                loader=part_loader, sample_num=sample_num, device=self._device,
