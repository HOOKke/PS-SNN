...
# BAR plotting ##################
#################################
# Generate the bar-figure
def plot_bar(
    # Source data and error #######
    numbers, yerr=None,
    # Setting of the plotting #####
    names=None, colors=None, ylabel=None, title=None, top_title=None, ylim=None, figsize=None,
):
    n_bars = len(numbers)

    size = (12, 7) if figsize is None else figsize
    f, axe = plt.subplots(1, 1, figsize=size)
    axe.bar(x=range(n_bars), height=numbers, color=colors, yerr=yerr, edgecolor='black')

    # deploy layout
    axe.set_xticks(range(n_bars))
    if names is not None:
        axe.set_xticklabels(names, rotation=-20)
        axe.legend()
...
# Generate the figure containing multiple bar-figure
def plot_bars(
    # Source data and error #######
    number_list, yerr=None,
    dots=None,  # -> the Scatter Points corresponding to each bar
    # Setting of the plotting #####
    names=None, colors=None, ylabel=None, title_list=None, top_title=None, ylim=None, figsize=None,
    vlines=None, alpha=None,
    ...
    # -> decide the Y-axis upper value
    maxY = np.max([np.max(list) for list in number_list])  # -> the upper value
    if dots is not None:
        maxY = np.max([np.max([np.max(list) for list in list_of_list]) for list_of_list in dots])
    y_max = maxY + 0.07 * maxY

    # Make figure slot
    size = (16, 7) if figsize is not None else figsize
    f, axarr = plt.subplots(1, n_plots, figsize=size)

    # make all plots
    for i in range(n_plots):
        axarr[i].bar(x=range(n_bars[i]), height=number_list[i], color=colors[i] if type(colors) == list else colors,
        ...
        # Horizontal and Vertical auxiliary Line ###########
        ...
        # multiple horizontal lines, possibly with error-bars
...
# Generate the figure which one column contains more than one bar
def plot_combined_bars(
    # Source data and error #######
    number_list, yerr=None,
    # Setting of the plotting #####
    names=None, labels=None, colors=None, ylabel=None, title=None, top_title=None, ylim=None, figsize=None, alpha=None
):
    n_bars = len(number_list)
    assert n_bars >= 1
    n_column = len(number_list[0])
    bar_width = 1 / (n_bars + 1)
    bar_pos = []
    for i in range(n_bars):
        pos = [x + bar_width * i for x in range(n_column)]
        bar_pos.append(pos)
    xticks_pos = [r + (n_bars - 1)/2 * bar_width for r in range(n_column)]

    size = (12, 7) if figsize is None else figsize
    f, axe = plt.subplots(1, 1, figsize=size)
    for i in range(n_bars):
        axe.bar(x=bar_pos[i], height=number_list[i], color=colors[i] if type(colors) == list else colors,
                width=bar_width, yerr=yerr[i] if yerr is not None else None, alpha=alpha[i] if type(alpha) == list else alpha,
                edgecolor='black', label=labels[i] if type(labels) == list else labels)
...
def plot_lines(
    # Source data and error #######
    list_with_lines, list_with_errors=None,
    # Setting of the plotting #####
    x_axes=None, line_names=None, colors=None, title=None, title_top=None,
    xlabel=None, ylabel=None, ylim=None,
    figsize=None, errors="shaded", x_log=False, with_dots=False, linestyle='solid',
    h_line=None, h_label=None, h_error=None, h_lines=None, h_colors=None, h_labels=None, h_errors=None
):
...
def plot_lines_with_baselines(
    ...
    :param title:           <str> title of plot
    :param title_top:       <str> text to appear on top of the title
    :param with_dots:       <bool> whether to point the scatter on the plotting
    :return: f:             <figure>
    '''
    ...
    # The PART FOR THE TASK LINE (AXE1) ###########
    for task_id, name in enumerate(line_names):
...
# STACK-PLOT plotting ###########
#################################
# -> a stacked plotting figure!
def plot_stacked_area_chart(
    # Source data and error #######
    labels, stacked_info,
    # Setting of the plotting #####
    stack_name=None, colors=None, xlabel=None, ylabel=None, title=None,
    top_title=None, ylim=None, figsize=None
):

    size = (12, 7) if figsize is None else figsize
    f, axe = plt.subplots(1, 1, figsize=size)
    # plt.stackplot(labels, *[stacked_info[i] for i in range(stacked_info.shape[0])], labels=stack_name, colors=colors)
    axe.stackplot(labels, *[stacked_info[i] for i in range(stacked_info.shape[0])], labels=stack_name, colors=colors)
...
def plot_scatter_groups(
    ...
    x, y,
    # Setting of the plotting #####
    colors=None, ylabel=None, xlabel=None, title=None, top_title=None, names=None,
...
def plot_histogram(
    ...
    numbers, bins=30,
    # Setting of the plotting #####
    ylabel="frequency", xlabel=None, title=None, top_title=None, ylim=None, xlim=None,
    figsize=None
):
    ...
    size = (12,7) if figsize is None else figsize
    f, axarr = plt.subplots(1, 1, figsize=size)
    axarr.hist(x=numbers, bins=bins)
...
def plot_histograms(
    ...
    for i in range(plotting_num):
        axarr.hist(x=numbers[i], bins=bins, alpha=1/plotting_num)
...
def plot_matrix(
    ...
    array,
    # Setting of the plotting #####
    title=None, xlabel=None, ylabel=None, cmap=plt.cm.Blues, integers=False, figsize=None,
...
def plot_box(
    ...
    data,
    # Setting of the plotting #####
    title=None, xlabel=None, ylabel=None, integers=False, figsize=None,
    ylim=None, xticklabels=None,
):
    # make plot
    size = (12, 7) if figsize is None else figsize
    f, axarr = plt.subplots(1, 1, figsize=size)
    im = axarr.boxplot(data)
    ...
    if title is not None:
        axarr.set_title(title)
    axarr.set(xticks=np.arange(len(data)),
              xticklabels=np.arange(len(data)) if xticklabels is None else xticklabels)
...
# plot the images as a grid into [pdf]
...
def plot_ablation(dict_list1, dict_list2, names_1, names_2, colors_1, colors_2, seed_list, index=0, change_level=None,
                  ylabel=None, title=None, ylim=None, long_names_1=None, long_name_2=None, perc=False, neg=False):
    dict_ids1 = list(range(len(dict_list1)))
    dict_ids2 = list(range(len(dict_list2)))
    dots1 = [[dict_list1[idx][seed][index] for seed in seed_list] for idx in dict_ids1]
    means1 = [np.mean([dict_list1[idx][seed][index] for seed in seed_list]) for idx in dict_ids1]
    dots2 = [[dict_list2[idx][seed][index] for seed in seed_list] for idx in dict_ids2]
    means2 = [np.mean([dict_list2[idx][seed][index] for seed in seed_list]) for idx in dict_ids2]

    if len(seed_list) > 1:
        sems1 = [np.sqrt(np.var([dict_list1[idx][seed][index] for seed in seed_list]) / (len(seed_list) - 1)) for idx in dict_ids1]
        sems2 = [np.sqrt(np.var([dict_list2[idx][seed][index] for seed in seed_list]) / (len(seed_list) - 1)) for idx in dict_ids2]

    figure = plot_bars(
        number_list=[means1, means2], names=[names_1, names_2], colors=[colors_1, colors_2], ylabel=ylabel,
        yerr=[sems1, sems2] if len(seed_list) > 1 else None, ylim=ylim, top_title=title,
        title_list=["", ""], vlines=[0.5, 0.5], alpha=[0.7, 1.],
        dots=[dots1, dots2] if len(seed_list) > 1 else None,
        h_line=change_level, h_labels="change_level" if change_level is not None else None,
    )
    # print results to screen
    for i, name in enumerate(names_1):
        if len(seed_list) > 1:
            print("{:26s} {:9.2f}  (+/- {:6.2f}),  n={}".format(name, 100*means1[i], 100*sems1[i], len(seed_list)))
        else:
            print("{:30s} {:9.2f}".format(name, means1[i]))
    print("-" * 60)
    for i, name in enumerate(names_2):
        if len(seed_list) > 1:
            print("{:26s} {:9.2f}  (+/- {:6.2f}),  n={}".format(name, 100*means2[i], 100*sems2[i], len(seed_list)))
        else:
